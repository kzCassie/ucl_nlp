...
class GRUParser(nn.Module):
    ...
    def __init__(self, args, vocab, transition_system):
        ...
        # LSTMs
        if args.lstm == 'lstm':
            self.encoder_lstm = nn.GRU(args.embed_size, int(args.hidden_size / 2), bidirectional=True)
    ...
    def encode(self, src_sents_var, src_sents_len):
        ...
        src_encodings, last_state = self.encoder_lstm(packed_src_token_embed) ##########
    ...
    def score(self, examples, return_encode_state=False):
        ...
        src_encodings, last_state = self.encode(batch.src_sents_var, batch.src_sents_len)
